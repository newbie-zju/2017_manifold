...
bool DpTouching::calculateTrajectoryCallback(iarc_mission::TG::Request &req, iarc_mission::TG::Response &res)
{
	ros::spinOnce();
	float k = 0.7;
	float tarVz = k * (1.8 - quadrotorPosNED.z);
...
// 			runMethod();
		...
		case TG_TRACK:
		{
			...
			if (req.targetPosNEDz > 1.2)
			{
				...
				res.flightCtrlDstx = min(0.6*(tarX-quadrotorPosNED.x),0.7);
				res.flightCtrlDsty = min(0.6*(tarY-quadrotorPosNED.y),0.7);
				res.flightCtrlDstz = tarVz;
			}
			else
			{
				
				tarX = req.targetPosNEDx + 0.7 * cos(req.theta) - 0.23;
		...
		case TG_APP_HEAD:
		{
			...
			tarX = req.targetPosNEDx + 1.2 * cos(req.theta);
			tarY = req.targetPosNEDy + 1.2 * sin(req.theta);
			tarZ = -0.5;
			...
			res.flightCtrlDstx = 0.53*(tarX-quadrotorPosNED.x);//+quadrotorPosNED.x;
			res.flightCtrlDsty = 0.53*(tarY-quadrotorPosNED.y);//+quadrotorPosNED.y;

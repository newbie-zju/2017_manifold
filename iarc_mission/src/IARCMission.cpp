...
void IARCMission::initialize()
{
	...
	*/
	...
	GlbVision_P1 = {12,8,CruiseHeight};
	GlbVision_P2 = {10,5,CruiseHeight};
	GlbVision_P3 = {10,11,CruiseHeight};

	maxWaitTime = 10;
	testInteractionFlag = false;
	testObsFlag = false;
	...
	cruiseGreenSetPY = {8,5,11,2,14,8,5,11,2,14};
...
void IARCMission::irobot_pos_callback(const irobot_detect::Pose3DConstPtr& msg)
{
	...
	if(msg->flag > 0)
	{
		...
		if (fabs(limitAng(irobotPosNED.theta - limitAng(yaw_origin*M_PI/180.0))) < M_PI/4.0 || fabs(limitAng(irobotPosNED.theta - limitAng(yaw_origin*M_PI/180.0 + M_PI))) < M_PI/4.0)
			rightDirection = true;
		else
...
void IARCMission::boundaryDetect_callback(const geometry_msgs::PointStampedConstPtr& msg)
{
	if(msg->point.z > 0)
	{
		...
		if (quadrotorGroundPos.x < 1.5 && quadrotorGroundPos.x >0)
		{
			greenBoundaryClose = true;
		}
		else
			greenBoundaryClose = false;
	}
	else
	{
		boundaryEmergency = false;
...
void IARCMission::quadrotorPosGroundCallback(const geometry_msgs::PointStampedConstPtr& msg)
{
	...
	ROS_INFO_THROTTLE(0.5,"Ground position x = %3.1f, y = %3.1f",quadrotorGroundPos.x,quadrotorGroundPos.y);
}
...
void IARCMission::missionCruise_P1()
{
	...
	while(ros::ok() && quadState == CRUISE_P1)
	{
		...
		ROS_INFO_THROTTLE(1,"quadGroundX = %3.1f, quadGroundY = %3.1f, setNEDX = %3.1f, setNEDY = %3.1f",quadrotorGroundPos.x,quadrotorGroundPos.y,cruiseSetPX_NED,cruiseSetPY_NED);
...
void IARCMission::missionSearch_P1()
{
	...
	while (ros::ok() && quadState == SEARCH_P1)
	{
		...
		else if(irobotPosNED.flag > 0 && rightDirection)
		{
...
void IARCMission::missionCruise_TARGET()
{
	...
	while (ros::ok() && quadState == CRUISE_TARGET)
	{
		...
		if(getLength2f(localPosNED.x-targetSetX,localPosNED.y-targetSetY)<1 && irobotPosNED.flag>0 && rightDirection)
		{
		...
		else if(getLength2f(localPosNED.x-targetSetX,localPosNED.y-targetSetY)<1 && (!(irobotPosNED.flag>0)||!rightDirection))
		{
			quadState = WANDER_SEARCH;
			time_last = ros::Time::now();
		}
...
void IARCMission::missionCruise_GREEN()
{
	...
	while(ros::ok() && quadState == CRUISE_GREENBDR)
	{
		...
		else if (glbVisTargetFlag)
		{
			quadState = CRUISE_TARGET;
			time_last = ros::Time::now();
			ROS_INFO("JUMP from cruise_green to cruise_target");
		}
		else if(boundaryClose)
		{
			quadState = CRUISE_P1;
...
void IARCMission::missionWander()   //TODO: add the WANDER case in the trajectory generation package
{
	...
	while (ros::ok() && quadState == WANDER_SEARCH)
	{
		...
		if(irobotPosNED.flag > 0 && rightDirection)
		{
			quadState = TRACK_TARGET;
			time_last = ros::Time::now();
...
void IARCMission::missionTrack()
{
	...
	while(ros::ok() && quadState == TRACK_TARGET)
	{
	    ...
	    delta_x = 0;//(float)TG_srv.request.targetPosNEDx - localPosNED.x;
	    delta_y = 0;//(float)TG_srv.request.targetPosNEDy - localPosNED.y;
	    delta_z = 0;//(float)TG_srv.request.targetPosNEDz - localPosNED.z;
	    obs_msg.data.push_back(delta_x);
	    obs_msg.data.push_back(delta_y);
	    obs_msg.data.push_back(delta_z);
		...
		if (((irobotPosNED.flag == 0||!rightDirection) && duration.toSec()>1.5))
		{
			quadState = WANDER_SEARCH;
			time_last = ros::Time::now();
		...
		{
			if (greenBoundaryClose) // fabs(limitAng(irobotPosNED.theta - limitAng(yaw_origin*M_PI/180.0))) < M_PI/4.0
			{
				quadState = TRACK_TARGET;
				ROS_INFO("green boundary, keep tracking");
			}
			else
			{
				quadState = CRUISE_P1;
		...
		{
			if (greenBoundaryClose)
			{
				quadState = TRACK_TARGET;
				ROS_INFO("green boundary, approach awaiting");
			}
			else
			{
...
void IARCMission::missionApproach_HEAD()
{
	...
	while(ros::ok() && quadState == APPROACH_HEAD)
	{
		...
		else if(obstacle_emergency && obstacle_dist<1.8)
		{
			quadState = CRUISE_P1;
			time_last = ros::Time::now();
...
bool IARCMission::readytoHeadCollision()
{
	...
	if (inActionRange && fabs(limitAng(irobotPosNED.theta - limitAng(yaw_origin*M_PI/180.0))) < M_PI/4.0 && time_in_20s > 2.5 && time_in_20s < 7)
		return true;
	else if (inActionRange && fabs(limitAng(irobotPosNED.theta - limitAng(yaw_origin*M_PI/180.0 + M_PI))) < M_PI/4.0 && time_in_20s < 15 && time_in_20s >11.5)
		return true;
	else
		return false;
}
...
bool IARCMission::readytoTrackApproach()
{
	...
	if (localPosNED.z > 1.2)
		...
		inActionRange = (getLength2f(irobotPosNED.x+0.1*cos(irobotPosNED.theta)-0.23-localPosNED.x,irobotPosNED.y+0.1*sin(irobotPosNED.theta)-localPosNED.y) < 0.2);
...
bool IARCMission::readytoTopTouch()
{
	...
	if (localPosNED.z > 1.2)
		...
		//inActionRange = (getLength2f(irobotPosNED.x+0.6*cos(irobotPosNED.theta)+0.2*cos(irobotPosNED.theta+M_PI/2)-localPosNED.x,irobotPosNED.y+0.6*sin(irobotPosNED.theta)+0.2*sin(irobotPosNED.theta+M_PI/2)-localPosNED.y) < 0.6);
		inActionRange = (getLength2f(irobotPosNED.x+0.05*cos(irobotPosNED.theta)-0.23-localPosNED.x,irobotPosNED.y+0.05*sin(irobotPosNED.theta)-localPosNED.y) < 0.15);
	//inTopActionAngle = !(fabs(limitAng(irobotPosNED.theta - limitAng(yaw_origin*M_PI/180.0))) < M_PI/4.0 || fabs(limitAng(irobotPosNED.theta - limitAng(yaw_origin*M_PI/180.0 + M_PI))) < M_PI/4.0);
	inTime = ((time_in_20s>2.5) && (time_in_20s<18));

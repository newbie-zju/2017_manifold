...
private:
    ...
    cv::gpu::GpuMat GPU_src_img, GPU_undistor_img, GPU_map1, GPU_map2;
};
FisheyeModel::FisheyeModel(ros::NodeHandle nh):nh_(nh),nh_param("~")
{
    cv::FileStorage fs("/home/ubuntu/2017/src/fisheye/data/camera.yml", cv::FileStorage::READ);
...
void FisheyeModel::initialize()
{
   ...
   // std::cout << focalMultiple << std::endl;
    ...
    FisheyeModel::initUndistortRectifyMap(intrinsicMatrix, distortionCoeffs, R, new_intrinsic_mat, size,  CV_32FC1, map1, map2);
    ...
    const cv::Size& size, int m1type, cv::Mat & map1, cv::Mat & map2 )
{
    ...
    //ROS_INFO("frequency: %5.4f", 1.0f/ros::Duration(ros::Time::now()-start).toSec());
}

void FisheyeModel::undistortImage(cv::Mat & distorted, cv::Mat & undistorted)
{
    cv::Mat src_img_4, distor_img_4, map1_4, map2_4;
    cv::cvtColor(distorted,src_img_4,CV_BGR2BGRA);//gpu
    GPU_src_img.upload(src_img_4);//gpu
    
    cv::gpu::remap(GPU_src_img, GPU_undistor_img,GPU_map1, GPU_map2, cv::INTER_LINEAR, cv::BORDER_CONSTANT);
...
void FisheyeModel::FishEyeImgUndistort(cv::Mat &src_img, cv::Mat &UndistortImg)
{
  ...
  /*  intrinsic_mat.copyTo(new_intrinsic_mat);
    ...
    new_intrinsic_mat.at<float>(0, 0) *= 1;
    new_intrinsic_mat.at<float>(1, 1) *= 1;
   // std::cout << focalMultiple << std::endl;
    
    new_intrinsic_mat.at<float>(0, 2) += 0;
    new_intrinsic_mat.at<float>(1, 2) += 0;*/
